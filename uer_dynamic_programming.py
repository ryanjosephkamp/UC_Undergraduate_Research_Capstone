...
For more details, please see my presentation of this paper on YouTube: 
https://youtu.be/LRAP1YHy3g0
...
Unknown Evidence Reliability ("UER")



FUNCTIONS:

1. uer_tdtc_dynpro__multi_cost_examples:
...
2. uer_tdtc_dynpro__multi_bib_example__tanh:
...
# Get transition probability (see paper or presentation for derivation) from current belief g_cur to next belief g_next at current time t:
def p_uer(g_cur, g_next, t, sigalpha2, dt):
  dt_eff = dt/(t + 1/sigalpha2)
  icdf_gnext = stats.norm.ppf(g_next)
  coeff = 1/np.sqrt(dt_eff)
  f = np.exp((icdf_gnext**2)/2 - ((icdf_gnext - np.sqrt(1 + dt_eff)*stats.norm.ppf(g_cur))**2)/(2*dt_eff))
  return coeff * f



# Get matrix ps of transition probabilities with current beliefs down columns and next beliefs across rows for current time t:
def get_ps_uer(gs, t, M_dynpro, sigalpha2, dt):
  return np.array([[p_uer(gs[j], gs[k], t, sigalpha2, dt) for j in range(M_dynpro)] for k in range(M_dynpro)])
...
# Transform belief g into corresponding accumulated evidence X:
def g_to_X_uer(g, sigalpha2, t):
  return np.sqrt(t + 1/sigalpha2)*stats.norm.ppf(g)


# Transform accumulated evidence X into corresponding belief g:
def X_to_g_uer(X, sigalpha2, t):
  return stats.norm.cdf(X/np.sqrt(t + 1/sigalpha2))


# Transform the upper and lower g-bounds into corresponding X-bounds:
def get_uer_Xbounds(sigalpha2, gH, gL, t):
  return g_to_X_uer(gH, sigalpha2, t), g_to_X_uer(gL, sigalpha2, t)



# Generate the acc. evidence and belief stochastic process data:
def generate_data_uer(z, g_0, sigalpha2, dt, M_ts, ts):
  ...
  Xs[0] = g_to_X_uer(g_0, sigalpha2, 0)

  # Populate Xs and gs arrays:
  for n in range(1, M_ts):
    # Draw the absolute evidence reliability parameter alpha from a normal distribution with mean 0 and variance sigalpha2:
    alpha = np.abs(np.random.normal(0, np.sqrt(sigalpha2))) # the corresponding mean is mu = alpha * z

    # Accumulate evidence from a normal distribution with mean mu*dt and variance dt:
    Xs[n] = Xs[n-1] + np.random.normal(z*alpha*dt, np.sqrt(dt))
    gs[n] = X_to_g_uer(Xs[n], sigalpha2, ts[n])
...
# Get the optimal X- and g-bounds at the current time and cost values:
def uer_ctc__dynpro_bounds(sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, t, c, anim):
  ...
  while True:
    ...
    # Try to run the algorithm with the given belief discretization number M_dynpro; loop will repeat with increased M_dynpro if M_dynpro is too small for rootfinding:
    try:
      ...
      ps = get_ps_uer(gs, t, M_dynpro, sigalpha2, dt)
    ...
    except ValueError: # insufficient number of discrete beliefs for rootfinding to work

      # Increase M_dynpro (belief discretization number) by 2 and try again:
      M_dynpro += 2
  ...
  if anim:
    ...
    ani.save(str(M_dynpro) + 'beliefs__' + str(sigalpha2) + '_sigalpha2__' + str(dt) + '_dt__' + str(c) + '_c__' + str(tol) + '_tol___' + 'TIME_' + str(t) + '.mp4', writer=writer)

  # Define the values to return:
  gH = gHs[-1] # the convergent upper g-bound
  gL = gLs[-1] # the convergent lower g-bound
  XH, XL = get_uer_Xbounds(sigalpha2, gH, gL, t) # the corresponding convergent upper and lower X-bounds

  return XH, XL, gH, gL, M_dynpro



# Compute the UER bound function arrays:
def uer__dynpro_bound_functions(sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c):
  ...
  *** RECOMMENDATION:    use M_dynpro = 19 for sigalpha2 = 1, dt = 0.001, interp_kind = 'quadratic', and root_method = 'brentq' ***
  ...
  # Initialize NumPy array for working discretization numbers M_dynpro:
  M_dynpros = np.zeros(M_ts)

  # Populate the bound arrays:
  for n in tqdm(range(M_ts)):
    XHs[n], XLs[n], gHs[n], gLs[n], M_dynpros[n] = uer_ctc__dynpro_bounds(sigalpha2, dt, M_dynpro + (n//50)*2, tol, interp_kind, root_method, sfs, ts[n], c[n], True)

  return XHs, XLs, gHs, gLs, M_dynpros



# Plot UER trajectories for five example cost functions with unbiased initial beliefs:
def uer_tdtc_dynpro__multi_cost_examples(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts):
    ...
    *** RECOMMENDATION:    use M_dynpro = 19 for sigalpha2 = 1, dt = 0.001, interp_kind = 'quadratic', and root_method = 'brentq' ***
    ...
    Xfull, gfull = generate_data_uer(z, 0.5, sigalpha2, dt, M_ts, ts) # these will be modified for each cost function and stored in respective rows of Xs and gs
    ...
    for c_n in range(5):
      ...
      for n in tqdm(range(M_ts)):
        ...
        # Populate the X- and g-bound function rows (disregard dummy variable):
        XHs[c_n, n], XLs[c_n, n], gHs[c_n, n], gLs[c_n, n], dummy = uer_ctc__dynpro_bounds(sigalpha2, dt, M_dynpro + (n//50)*2, tol, interp_kind, root_method, sfs, ts[n], cs[c_n, n], False)
        ...
        if bc == XHs[c_n, n] or bc == XLs[c_n, n]:
          ...
          gs[c_n][n] = X_to_g_uer(Xs[c_n][n], sigalpha2, ts[n])
      ...
      plt.title('Accumulated Evidence ' + f'$X(t)$' + ' vs. Time ' + f'$t$' + ';\tCost Function ' + cfunstrs[c_n] + ';\tParameters: ' + f'$σ_α^2 = $' + str(sigalpha2) + ', ' + f'$dt = $' + str(dt))
      ...
      plt.title('Belief ' + f'$g(t)$' + ' vs. Time ' + f'$t$' + ';\tCost Function ' + cfunstrs[c_n] + ';\tParameters: ' + f'$σ_α^2 = $' + str(sigalpha2) + ', ' + f'$dt = $' + str(dt))
...
# Plot UER trajectories using cost function c(t)=tanh(t) for a total of M_gs initial beliefs between g_0_start and g_0_stop:
def uer_tdtc_dynpro__multi_bib_example__tanh(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, g_0_start, g_0_stop, M_gs, XHs, XLs, gHs, gLs, M_dynpros):
  ...
  - XHs:                 upper X-bound function NumPy array of length M_ts

  - XLs:                 lower X-bound function NumPy array of length M_ts

  - gHs:                 upper g-bound function NumPy array of length M_ts

  - gLs:                 lower g-bound function NumPy array of length M_ts

  - M_dynpros:           NumPy array of minimum numbers of discrete beliefs needed for rootfinding to work
  ...
  for g_n in tqdm(range(M_gs)):
    ...
    Xs[g_n], gs[g_n] = generate_data_uer(z, g_0s[g_n], sigalpha2, dt, M_ts, ts)
    ...
    for n in range(M_ts):
      ...
      if bc == XHs[n] or bc == XLs[n]:
        ...
        # Update the current belief to that corresponding to the bound that was hit:
        gs[g_n][n] = X_to_g_uer(Xs[g_n][n], sigalpha2, ts[n])
  ...
  # Plot working belief discretization number vs. initial belief:
  fig, ax = plt.subplots()
  plt.rcParams['font.size'] = 20
  fig.set_size_inches(24, 14)

  ax.plot(ts, M_dynpros, lw=3)

  ax.set_xlabel('Time, ' + f'$t$')
  ax.set_ylabel('Working Belief Discretization Number, ' + f'$M_{{dynpro}}$')

  plt.title('Working Belief Discretization Number ' + f'$M_{{dynpro}}$' + ' vs. Time ' + f'$t$' + ' for ' + str(M_ts) + ' Times Between ' + str(ts[0]) + ' and ' + str(ts[-1]) + '\nCost Function ' + cfunstr + ';\tParameters: ' + f'$σ_α^2 = $' + str(sigalpha2) + ', ' + f'$dt = $' + str(dt))

  plt.show()
  """____________________________________________________________________________________________________"""

  return g_0s, ts, cs, Xs, gs, XHs, XLs, gHs, gLs, M_dynpros, nstops, tstops





# Plot UER trajectories together using cost function c(t)=tanh(t) for a total of M_gs initial beliefs between g_0_start and g_0_stop for five example initial belief ranges and numbers:
def uer_tdtc_dynpro__multi_bib__tanh_examples(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts):
  ...
  # Get the upper X-, lower X-, upper g-, and lower g-bound function NumPy arrays; also get working belief discretization numbers (M_dynpros):
  XHs, XLs, gHs, gLs, M_dynpros = uer__dynpro_bound_functions(sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, np.tanh(ts))
  # (this function only has to be called once, and the generated bound functions are used for all five examples)

  # Generate plots for the five examples:
  uer_tdtc_dynpro__multi_bib_example__tanh(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, 0.25, 0.75, 5, XHs, XLs, gHs, gLs, M_dynpros)
  uer_tdtc_dynpro__multi_bib_example__tanh(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, 0.1, 0.9, 10, XHs, XLs, gHs, gLs, M_dynpros)
  uer_tdtc_dynpro__multi_bib_example__tanh(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, 0.01, 0.99, 100, XHs, XLs, gHs, gLs, M_dynpros)
  uer_tdtc_dynpro__multi_bib_example__tanh(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, 0.001, 0.999, 1000, XHs, XLs, gHs, gLs, M_dynpros)
  uer_tdtc_dynpro__multi_bib_example__tanh(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, 0.0001, 0.9999, 10000, XHs, XLs, gHs, gLs, M_dynpros)

  return None



# Plot UER trajectories using specified constant cost 0 < c < 1 for a total of M_gs initial beliefs between g_0_start and g_0_stop:
def uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, g_0_start, g_0_stop, M_gs, XHs, XLs, gHs, gLs, M_dynpros):
  ...
  # Get NumPy array of chosen trial cost function (here, c(t) = c) outputs over time input array ts:
  cs = c*np.ones(M_ts)
...
# Plot UER trajectories together using constant cost function c(t) = c for a total of M_gs initial beliefs between g_0_start and g_0_stop for five example initial belief ranges and numbers:
def uer_ctc_dynpro__multi_bib__examples(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, c):


  # Get NumPy array of M_ts total times of increment size (trial duration) dt; empirically, M_ts >= 1,000 appears to be sufficient for dt >= 0.001
  ts = np.arange(0, M_ts*dt, dt)

  """___ Generate the X- and g-bound functions from the chosen cost function and parameters (sig2, dt): ___"""
  # Get the upper X-, lower X-, upper g-, and lower g-bound function NumPy arrays; also get working belief discretization numbers (M_dynpros):
  XHs, XLs, gHs, gLs, M_dynpros = uer__dynpro_bound_functions(sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c*np.ones(M_ts))
  # (this function only has to be called once, and the generated bound functions are used for all five examples)

  # Generate plots for the five examples:

  uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, 0.25, 0.75, 5, XHs, XLs, gHs, gLs, M_dynpros)
  uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, 0.1, 0.9, 50, XHs, XLs, gHs, gLs, M_dynpros)
  uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, 0.01, 0.99, 100, XHs, XLs, gHs, gLs, M_dynpros)
  uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, 0.001, 0.999, 1000, XHs, XLs, gHs, gLs, M_dynpros)
  uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, 0.0001, 0.9999, 10000, XHs, XLs, gHs, gLs, M_dynpros)

  return None



# Plot UER trajectories using a specified constant cost 0 < c < 1 for a single unbiased initial belief g_0 = 0.5:
def uer_ctc_dynpro__uib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, c):
  ...
  - M_ts:                number of discrete times; empirically, M_ts >= 1,000 appears to be sufficient for dt >= 0.001
  ...
  return uer_ctc_dynpro__multi_bib(z, sigalpha2, dt, M_dynpro, tol, interp_kind, root_method, sfs, M_ts, ts, c, 0.5, 0.5, 1, XHs, XLs, gHs, gLs, M_dynpros)
...
#uer_tdtc_dynpro__multi_cost_examples(1, 1, 0.001, 35, 1e-3, 'quadratic', 'brentq', 5, 100)

#uer_tdtc_dynpro__multi_bib__tanh_examples(1, 1, 0.001, 19, 1e-3, 'quadratic', 'brentq', 5, 1000)

#uer_ctc_dynpro__multi_bib__examples(1, 1, 0.001, 19, 1e-3, 'quadratic', 'brentq', 5, 1000, 0.2)

#uer_ctc_dynpro__uib(1, 1, 0.001, 19, 1e-3, 'quadratic', 'brentq', 5, 100, 0.2)
